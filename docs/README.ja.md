...
レガシー実装(v0.2.1-)と現行パイプライン(v1.0.0+)を比較するには、同梱のヘルパースクリプトを利用してください。依存関係と仮想環境を揃えるため、`uv` 経由での実行を推奨します。
...
安定版は PyPI（v1.0.0）で公開済みで、今後のアップデートに向けて継続的に改善を進めています。新しいタグとチェンジログをお見逃しなく。
