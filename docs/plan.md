...
- [x] 実装: `Step` プロトコルと `Pipeline` 本体を最小限実装し、テストをパスさせる。
- [x] リファクタリング: 結果クラスや例外設計を整理し、テストを通す。

### step2

- [x] テスト: `Resize`/`Grayscale`/`Rotate`/`Flip` の入出力仕様を定義したテストを追加（サイズ・モード・回転角・反転方向を検証）。
- [x] 実装: 各変換ステップクラスを実装してテストを通す。
- [x] リファクタリング: 変換ステップで共有するユーティリティやバリデーションを整理。

### step3

- [x] テスト: Otsu 法と固定閾値を切り替えるテストを追加し、閾値引数のバリデーションを確認。
- [x] 実装: `BinarizeStep` を実装し、設定値に応じて正しく2値化されるようにする。
- [x] リファクタリング: 2値化ステップの設定やエラーハンドリングを共通化。

### step4

- [x] テスト: メディアン／バイラテラルフィルタ適用後のノイズ低減を検証し、副作用（サイズ・型）が変わらないことを確認。
- [x] 実装: `DenoiseStep` を実装し、閾値処理や他ステップと組み合わせた動作を保証する。
- [x] リファクタリング: ノイズ除去のパラメータ管理とドキュメントを整備。

### step5

- [x] 実装: README/plan の最終更新、サンプルフォルダの整備。
- [x] リファクタリング: 不要な TODO や重複を削除し、CI でテストをパスさせる。
